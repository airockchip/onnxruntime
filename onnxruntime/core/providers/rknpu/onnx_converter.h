...
class OnnxConverter {
 private:
  ...
  CreateRknnTensor(const std::string name, const std::vector<uint32_t> dims, const void* data = NULL,
  ...
  void AddLayerFlatten(const std::string& input, const int32_t axis,
                       const std::string& output);
  void AddLayerTranspose(const std::string& input, const std::vector<int32_t>& perm,
                         const std::string& output);
  void AddLayerSqueeze(const std::string& input, const std::vector<int32_t>& axes,
                       const std::string& output);
  void AddLayerUnsqueeze(const std::string& input, const std::vector<int32_t>& axes,
                         const std::string& output);
 ...
 public:
  ...
  std::vector<std::vector<int>> GetSupportedNodes(ONNX_NAMESPACE::ModelProto model_proto);
  void Convert(const ONNX_NAMESPACE::ModelProto& model, rk::nn::Graph* graph,
               std::vector<const void*> input_bufs, std::unordered_map<std::string, int> input_maps);
