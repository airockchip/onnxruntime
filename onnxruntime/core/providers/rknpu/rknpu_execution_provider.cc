...
std::vector<std::vector<int>> RknpuExecutionProvider::GetSupportedNodes(const ONNX_NAMESPACE::ModelProto& model_proto) const {
...
RknpuExecutionProvider::GetCapability(const onnxruntime::GraphViewer& graph_viewer,
  ...
  for (const auto& tensor : graph_viewer.GetAllInitializedTensors()) {
    if (tensor.second->has_data_location() && tensor.second->data_location() == ONNX_NAMESPACE::TensorProto_DataLocation_EXTERNAL) {
      LOGS_DEFAULT(WARNING) << "Rknpu: Initializers with external data location are not currently supported";
      return result;
    }
  }
...
common::Status RknpuExecutionProvider::Compile(const std::vector<onnxruntime::Node*>& fused_nodes,
  ...
  for (const auto* fused_node : fused_nodes) {
    ...
    *(model_proto.mutable_graph()) = graph_body.ToGraphProto();
    ...
    compute_info.create_state_func = [&](ComputeContext* context, FunctionState* state) {
      std::unique_ptr<RknpuFuncState> p = onnxruntime::make_unique<RknpuFuncState>();
      rk::nn::Graph* graph = new rk::nn::Graph();
      *p = {"", std::unique_ptr<rk::nn::Exection>(new rk::nn::Exection(graph)),
            model_proto_[context->node_name], input_info_[context->node_name], output_info_[context->node_name],
    ...
    compute_info.compute_func = [](FunctionState state, const OrtCustomOpApi* api, OrtKernelContext* context) {
      ...
      for (size_t i = 0; i < n_inputs; i++) {
        ...
        input_shape.append(reinterpret_cast<const char*>(tensor_shape.data()), ndim * sizeof(int64_t));
      }
      ...
      if (s->uniq_input_shape == "") {
        ...
        for (size_t i = 0; i < n_inputs; i++) {
          const OrtValue* input_tensor = ort.KernelContext_GetInput(context, i);
          input_bufs[i] = const_cast<const void*>(ort.GetTensorData<void>(input_tensor));
          auto tensor_info = ort.GetTensorTypeAndShape(input_tensor);
          auto tensor_shape = ort.GetTensorShape(tensor_info);
          ort.ReleaseTensorTypeAndShapeInfo(tensor_info);
        ...
        // TODO
        throw std::invalid_argument("The input_shape is not match the s->uniq_input_shape!");
      } else {
        LOGS_DEFAULT(INFO) << "input_shape equal to s->uniq_input_shape, skip rebuild!";
      }
      ...
      ORT_ENFORCE(graph->GetInputs().size() <= n_inputs, "Inconsistent input sizes");
      ORT_ENFORCE(graph->GetOutputs().size() == n_outputs, "Inconsistent output sizes");

      std::vector<rk::nn::InputInfo> inputs;
      inputs.resize(graph->GetInputs().size());
      for (size_t i = 0; i < graph->GetInputs().size(); i++) {
        const OrtValue* input_tensor = ort.KernelContext_GetInput(context, s->input_indexes[i]);
        float* input_buf = const_cast<float*>(ort.GetTensorData<float>(input_tensor));
        ...
        switch (type) {
          ...
          default:
            // TODO
            throw std::invalid_argument("compute_func: unknow input data type!");
            break;
        }

        inputs[i].index = i;
        inputs[i].buf = (void*)input_buf;
        inputs[i].size = std::accumulate(tensor_shape.begin(), tensor_shape.end(), 1, std::multiplies<int64_t>()) * type_size;
      ...
      for (size_t i = 0; i < n_outputs; i++) {
        ...
        auto* output_tensor = ort.KernelContext_GetOutput(context, s->output_indexes[i], int64_output_shape.data(), int64_output_shape.size());
        float* output_buf = const_cast<float*>(ort.GetTensorData<float>(output_tensor));

        auto type = output->GetPrecision();
        size_t type_size = 4;
        switch (type) {
          case rk::nn::PrecisionType::FLOAT32:
            type_size = 4;
          ...
          default:
            // TODO
            throw std::invalid_argument("compute_func: unknow output data type!");
            break;
        }

        outputs[i].index = i;
        outputs[i].buf = (void*)output_buf;
        outputs[i].size = accumulate(output_shape.begin(), output_shape.end(), 1, std::multiplies<uint32_t>()) * type_size;
        outputs[i].type = type;
...
std::shared_ptr<KernelRegistry> RknpuExecutionProvider::GetKernelRegistry() const {
  static std::shared_ptr<KernelRegistry> kernel_registry = onnxruntime::GetRknpuKernelRegistry();
  return kernel_registry;
}
