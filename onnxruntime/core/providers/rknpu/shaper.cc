...
void Shaper::Conv(const std::string& input, const std::string& weight,
                  ...
                  int32_t stride_y, const std::string auto_pad, const std::string& output) {
...
void Shaper::Conv(const std::string& input_name, int32_t strideX,
  ...
  if (auto_pad == "VALID") {
    ...
    int32_t pad_needed_Y = (legacy_target_size_Y - 1) * strideY + weightDimen[2] - inputDimen[2];
    int32_t pad_needed_X = (legacy_target_size_X - 1) * strideX + weightDimen[3] - inputDimen[3];
    Shape outputDimen{inputDimen[0],
                      weightDimen[0],
...
void Shaper::Slice(const std::string& input_name,
  ...
  for (size_t i = 0; i < axes.size(); i++) {
    int32_t axis = (axes[i] < 0) ? (axes[i] + (int32_t)inputDimen.size()) : axes[i];
...
void Shaper::Eltwise(const std::string& input1_name,
  ...
  for (int i = (int)max_shape.size() - 1, j = (int)min_shape.size() - 1; i >= 0 && j >= 0; i--, j--) {
...
void Shaper::Reshape(const std::string& input_name,
                     ...
                     const std::string& output_name) {
  auto input_dimen = shape_map_.at(input_name);
  int64_t input_size = std::accumulate(input_dimen.begin(), input_dimen.end(), 1, std::multiplies<uint32_t>());
  ...
  for (size_t i = 0; i < shape.size(); i++) {
    if (shape[i] == -1) {
      if (unk_dim_idx != -1)
        throw std::invalid_argument("Only one input dimension of Attr(shape) can be unknown!");
      unk_dim_idx = i;
    } else if (shape[i] == 0) {
      if (i >= input_dimen.size())
        throw std::invalid_argument("The index of dimension to copy from input shape must be less than the size of input shape!");
    } else {
      if (shape[i] < 0)
        throw std::invalid_argument("Each input dimension of Attr(shape) must not be negtive except one unknown dimension!");
    }
...
void Shaper::Squeeze(const std::string& input_name, const std::vector<int32_t>& axes,
                     const std::string& output_name) {
  vector<uint32_t> inputDimen = shape_map_.at(input_name);
  ...
  if (n_axes == 0) {
    ...
    for (size_t idx = 0; idx < n_axes; ++idx) {
      int32_t current = axes[idx] < 0 ? axes[idx] + inputDimen.size() : axes[idx];
...
void Shaper::Unsqueeze(const std::string& input_name, const std::vector<int32_t>& axes,
                       const std::string& output_name) {
  vector<uint32_t> inputDimen = shape_map_.at(input_name);
